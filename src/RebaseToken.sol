...
contract RebaseToken is ERC20, Ownable, AccessControl {
    ...
    mapping(address => uint256) private sUserInterestRate; // Keeps track of the interest rate of the user at the time they last deposited, bridged or were transferred tokens.
    mapping(address => uint256) private sUserLastUpdatedTimestamp; // the last time a user balance was updated to mint accrued interest.
    uint256 private sInterestRate = 5e10; // this is the global interest rate of the token - when users mint (or receive tokens via transferral), this is the interest rate they will get.
    ...
    /**
     * @notice Grants the mint and burn role to an address. This is only called by the protocol owner.
    ...
    /**
     * @notice Returns the principal balance of the user. The principal balance is the last
     * updated stored balance, which does not consider the perpetually accruing interest that has not yet been minted.
     * @param _user the address of the user
     * @return balanceOf principal balance of the user.
     * 
     */
    ...
    * @param _userInterestRate The interest rate of the user. This is either the contract interest rate if the user is depositing or the 
    * user's interest rate from the source token if the user is bridging.
    ...
    */
     ...
     * @return balance total balance of the user.
     * 
     */
    ...
    /**
     * @notice Transfers tokens from the sender to the recipient. This function also mints any accrued interest 
     * since the last time the user's balance was updated.
    ...
    /**
     * @notice Transfers tokens from the sender to the recipient. This function also mints any accrued interest since the 
     * last time the user's balance was updated.
    ...
    /**
     * @notice Returns the interest accrued since the last update of the user's balance - 
    ...
    function _calculateUserAccumulatedInterestSinceLastUpdate(address _user)
        ...
        returns (uint256)
    {
        uint256 timeDifference = block.timestamp - sUserLastUpdatedTimestamp[_user];
        // represents the linear growth over time = 1 + (interest rate * time)
        return (sUserInterestRate[_user] * timeDifference) + PRECISION_FACTOR;
    }
     ...
     * @return sInterestRate the global interest rate of the token
     *
     */
