...
contract RebaseTokenTest is Test {
    ...
    function addRewardsToVault(uint256 amount) public {
        ...
        if (!success) revert("Failed to Add Rewards to Vault");
    }
    ...
    function testgrantMintAndBurnRole() public {
        ...
        // check someAddress doesn't have the role
        vm.prank(someAddress);
        vm.expectRevert();
        rebaseToken.mint(someAddress, SEND_VALUE, interestRate);
        // grant the role to someAddress
        vm.prank(owner);
        rebaseToken.grantMintAndBurnRole(someAddress);
        // check someAddress can now mint
        vm.prank(someAddress);
        rebaseToken.mint(someAddress, SEND_VALUE, interestRate);
    }
